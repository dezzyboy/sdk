...
export namespace SwapPools {
  ...
  export class ETHSwapToken extends SwapToken {
    ...
    constructor(args: ETHSwapTokenArgs) {
      let { swapEthAddress, nativeTokens, depositTokens, ...constructorArgs } =
        args;
  ...
  const ETHTokensPool = (t: Token): Token[] => [Tokens.NETH, t],
  ...
  export const GOD_POOL_SWAP_TOKEN = makeSwapToken({
    chainId: ChainId.GODWOKEN,
    address: "0xFb29599Dc6c6C1656d9cB4e37B880650EB863f69",
    netName: "Godwoken",
    poolId: 1,
    swapAddress: "0xB5C8084104b593fb9a045734621b9318EFc69074",
    poolTokens: [Tokens.USDC, Tokens.DAI, Tokens.BUSD],
  });
  export const FTMTEST_POOL_SWAP_TOKEN = makeSwapToken({
    chainId: ChainId.FTMTEST,
    address: "0x036CADABB29894988bA61Cb641C2b6CbC82Cea9f",
    netName: "Ftmtest",
    poolId: 1,
    swapAddress: "0x07A31A5798a54EC519246A651B091f60Ce8e8404",
    poolTokens: [Tokens.USDC, Tokens.DAI, Tokens.BUSD],
  });
  ...
  const AVAX_AVWETH_POOLTOKENS = ETHTokensPool(Tokens.AVWETH),
  ...
  const makeSingleTokenPool = (t: Token): LPToken => ({
    poolTokens: [t],
    swapType: t.swapType,
  });

  const ETH_Pool = makeSingleTokenPool(Tokens.ETH),
  ...
  const makeSwapTypeMap = (
    base: SwapTypeMapArgs,
    ...pools: LPToken[]
  ): BridgeTokenMapping => {
  ...
  export const bridgeSwappableMap: ChainSwapTypePoolsMap = {
    ...
    [ChainId.GODWOKEN]: makeSwapTypeMap({
      usdPool: [
        GOD_POOL_SWAP_TOKEN,
        GOD_POOL_SWAP_TOKEN.poolTokensForBridgeSwaps,
      ],
    }),
    [ChainId.FTMTEST]: makeSwapTypeMap({
      usdPool: [
        FTMTEST_POOL_SWAP_TOKEN,
        FTMTEST_POOL_SWAP_TOKEN.poolTokensForBridgeSwaps,
      ],
    }),
    ...
    [ChainId.METIS]: makeSwapTypeMap({
      usdPool: [
        ...
        METIS_POOL_SWAP_TOKEN.poolTokensForBridgeSwaps,
      ],
      ethPool: [
        METIS_ETH_SWAP_TOKEN,
        METIS_ETH_SWAP_TOKEN.poolTokensForBridgeSwaps,
      ],
    }),
    [ChainId.MOONBEAM]: makeSwapTypeMap({}, SOLAR_Pool, WMOVR_Pool, WAVAX_Pool),
    [ChainId.MOONRIVER]: makeSwapTypeMap({}, SOLAR_Pool, FRAX_Pool, MOVR_Pool),
    ...
    [ChainId.AURORA]: makeSwapTypeMap({
      usdPool: [
        ...
        AURORA_POOL_SWAP_TOKEN.poolTokensForBridgeSwaps,
      ],
      ohm: false,
    }),
  ...
  export function swapGroupsForChain(chainId: number): string[] {
    return Object.values(bridgeSwappableMap[chainId].swappableSwapGroups).map(
      (lp) => lp.swapType
    );
  }
  ...
  export function stableswapPoolForNetwork(chainId: number): SwapPoolToken {
    switch (chainId) {
      ...
      case ChainId.GODWOKEN:
        return GOD_POOL_SWAP_TOKEN;
      case ChainId.FTMTEST:
        return FTMTEST_POOL_SWAP_TOKEN;
...
export type AllNetworksSwappableTokensMap =
  ChainIdTypeMap<NetworkSwappableTokensMap>;

function filterGrps(
  chainAGrps: string[],
  chainBGrpsMap: SwapPools.SwapTypePoolTokens
): Token[] {
  let tokens: Token[] = [];
...
function swapGroupsLoop(
  ...
  supportedChainIds().forEach((chainId: number) => {
    ...
    res[chainId] = filterGrps(
      swapGrps,
      SwapPools.bridgeSwappableMap[chainId].swappableSwapGroups
    );
  });
 ...
 */
export function networkSwapTokensMap(
  chainIdA: number,
  chainIdB?: number
): NetworkSwappableTokensMap {
  let res: NetworkSwappableTokensMap = {};
  ...
  if (typeof chainIdB !== "undefined") {
    res[chainIdB] = filterGrps(
      swapGrpsA,
      SwapPools.bridgeSwappableMap[chainIdB].swappableSwapGroups
    );
  } else {
